...
Limitations:
The program does not yet add or remove contacts from the source file yet. The input file also does not support adding other information at the start of program yet (such as: school, major, work, date of birth). If a source file name 
that does not exist inside the project folder, the GUI will popup an alert that tells the user that the file does not exist. If the format is incorrect a similar popup will appear. [place any comments or notes that will help the 
grader here]
...
Future Works
The original idea for this project was to make an interactive graph of contacts. In the future, the application will contain a graph with contacts that point to their mutual friends. The contacts will also all save to a new output 
file as a local database. Additionally, there will be more formats to type in contact information instead of only allowing name, phone number, and photo to be in the input file. I would also like to make the project look nicer in 
the future after learning more about css styling.
